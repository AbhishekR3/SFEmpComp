"""
Project: San Francisco Employee Compensation Analysis
Authors: Abhishek Ramesh, Dominnic Chant
"""
import os
import numpy as np
import pandas as pd
...
def ScatterPlotMatrix():
    ...
    while OrgGroupPos<=5:
        ...
        plt.xlim(0, 658867)
        plt.ylim(0, 150000)
        plt.savefig('SFEmpComp/' + OrgGroup[OrgGroupPos] + '.png', bbox_inches='tight')
        plt.show()
        OrgGroupPos += 1
...
def JobCountPrediction():
    ...
    plt.legend((increasect, predictiongr), (('Increased 2013 -> 2019'), ('Predicted No.Employees in '+str(yearprediction))))
    fig.tight_layout()
    plt.tick_params(axis='x',labelsize=8.5)
    fig1 = plt.gcf()
    plt.show()
    fig1.savefig('SFEmpComp/PredictionBarPlot.png')
...
def generate_graphs(df):
    for i in range(2013,2020): 
        plotLines(df, i)
        plt.savefig('SFEmpComp/LineGraph' + str(i) + '.png', dpi = 200)
    # plt.show()
...
def DistinctJobHeatMap():
    ...
    fig.savefig("SFEmpComp/DistinctJobHeatBluesMap.png")
...
def GenerateHeatMap(df):
    ...
    plt.savefig('SFEmpComp/JobsHeatMap.png', dpi = 200)
